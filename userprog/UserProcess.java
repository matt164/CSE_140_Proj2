...
public class UserProcess {
    ...
    private int handleCreate(int myAddr){
    	int opened = -1;
    	String fileName = readVirtualMemoryString(myAddr, 256);
    	if(fileName != null){
    		for(int i = 2; i < 16; ++i){
    ...
    private int handleRead(int slotNum, int vAddr, int numBytes){
		...
		if(slotNum >= 0 && slotNum <= 15 && slotNum != 1){
			if(myFileSlots[slotNum] != null){
				if(myFileSlots[slotNum].read(buffer, 0, numBytes) > 0){
					readBytes = writeVirtualMemory(vAddr, buffer);
				}
			}
		}
		return readBytes;
	...
	private int handleWrite(int slotNum, int vAddr, int numBytes){
		...
		if(slotNum >= 1 && slotNum <= 15){
			if(myFileSlots[slotNum] != null){
				readFromMem = readVirtualMemory(vAddr, buffer, 0, numBytes);
				written = myFileSlots[slotNum].write(buffer, 0, readFromMem); 
				if(written >= 0){
					returnBytes = written;
				}
			}
